...
const ManageOffers = () => {
    ...
    const handleEditClick = async (offer) => {
        ...
        // Parse demo_screenshots (JSON string or comma-separated)
        let demos = [];
        if (offer.demo_screenshots) {
            if (Array.isArray(offer.demo_screenshots)) {
                demos = offer.demo_screenshots.filter(Boolean);
            } else if (typeof offer.demo_screenshots === 'string') {
                try {
                    const parsed = JSON.parse(offer.demo_screenshots);
                    demos = Array.isArray(parsed) ? parsed.filter(Boolean) : [];
                } catch (_) {
                    demos = offer.demo_screenshots.split(',').map(s => s.trim()).filter(Boolean);
                }
            }
        }
        setDemoScreenshots(demos);
                                        ...
                                        <div className="mt-4 space-y-4 bg-indigo-50/40 border-2 border-indigo-100 rounded-2xl p-5">
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setFormData({ ...formData, required_screenshot_count: e.target.value })}
